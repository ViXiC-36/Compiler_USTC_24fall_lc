  ...
  */
  ...
  if (_STR_EQ(n->name, "program")) {
    ...
    */
    ...
    auto compound_stmt_node =
        static_cast<ASTCompoundStmt *>(transform_node_iter(n->children[5]));
    node->compound_stmt = std::shared_ptr<ASTCompoundStmt>(compound_stmt_node);
    return node;
    ...
    */
    ...
    std::stack<syntax_tree_node *> s0;
    auto list_ptr0 = n->children[1];
    while (list_ptr0->children_num == 2) {
      s0.push(list_ptr0->children[1]);
      list_ptr0 = list_ptr0->children[0];
    }

    while (!s0.empty()) {
      auto child_node =
          static_cast<ASTVarDeclaration *>(transform_node_iter(s0.top()));
    ...
    /*
      selection-stmt -> if ( expression ) statement | if ( expression ) statement else statement ASTSelectionStmt的结构，需要填充的字段有
    ...
    */
    ...
    if (_STR_EQ(name, "expression") || _STR_EQ(name, "var") ||
      ...
      /*
        ...
        integer -> INTEGER
      */
      // 参照var-declaration的处理方式
      if (_STR_EQ(name, "integer")) {
        // 3.1
        int num = std::stoi(n->children[i]->children[0]->name);
        num_node->i_val = num;
        num_node->type = TYPE_INT;
      } else if (_STR_EQ(name, "float")) {
        // 3.2
        float num = std::stof(n->children[i]->children[0]->name);
        num_node->f_val = num;

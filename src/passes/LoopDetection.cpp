...
void LoopDetection::discover_loop_and_sub_loops(BasicBlock *bb, BBset &latches,
    ...
    while (!work_list.empty()) { // 当工作表非空时继续处理
        ...
        // TODO-2: 处理已属于其他循环的节点
            ...
            // 找到sub_loop的最顶层父循环
            auto ancestor_loop = sub_loop;
            while (ancestor_loop->get_parent() != nullptr) {
                ancestor_loop = ancestor_loop->get_parent();
            }
            // 已经处于当前循环
            if (ancestor_loop == loop) {
                continue;
            }
            // 循环嵌套
            loop->add_sub_loop(ancestor_loop);
            ancestor_loop->set_parent(loop);
            for (auto &bbb : ancestor_loop->get_blocks()) {
                loop->add_block(bbb);
                bb_to_loop_[bbb] = loop;
            }
            for (auto &pred : ancestor_loop->get_header()->get_pre_basic_blocks()) {
                work_list.push_back(pred);
            }
        }
    }
